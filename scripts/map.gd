...
func generate_map() -> void:
	for y in range(map_size.y):
		for x in range(map_size.x):
			...
			var symbol_label = tile_instance.get_node("TileButton/SymbolLabel")
			symbol_label.text = tile_data.symbol  # Replace with biome symbol (e.g., SYMBOL_DUNE, SYMBOL_FOREST, etc.)
			...
			# Connect mouse signals for the tooltip
			var tile_button = tile_instance.get_node("TileButton")
			tile_button.connect("mouse_entered", Callable(self, "_on_tile_mouse_entered").bind(x, y, tile_button))
			tile_button.connect("mouse_exited", Callable(self, "_on_tile_mouse_exited"))

			# Connect the press event to handle player movement
			tile_button.connect("pressed", Callable(self, "_on_tile_pressed").bind(x, y))
